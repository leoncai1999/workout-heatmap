...
class Routes extends Component {
  ...
  getSimilarRoutes() {
    ...
    for (let i = 0; i < user_activities.length; i++) {
      if (
        user_activities[i]["map"]["summary_polyline"] !== null &&
        user_activities[i]["map"]["summary_polyline"] !== undefined
      ) {
        ...
        for (let j = 0; j < route_groupings.length; j++) {
          ...
          if (
            Math.abs(
              route_groupings[j][0]["distance"] - user_activities[i]["distance"]
            ) <= 300
          ) {
            if (
              Math.abs(
                route_groupings[j][0]["total_elevation_gain"] -
                  user_activities[i]["total_elevation_gain"]
              ) <= 40
            ) {
              if (
                this.isSimilarRoute(
                  route_groupings[j][0]["map"]["summary_polyline"],
                  user_activities[i]["map"]["summary_polyline"]
                )
              ) {
  ...
  isSimilarRoute = (route1, route2) => {
    ...
    while (
      !start_found &&
      idx_1 < shorter_route.length &&
      idx_2 < longer_route.length
    ) {
      var curr_start_dist = geolib.getDistance(
        shorter_route[idx_1],
        longer_route[idx_2]
      );
          ...
          <Card className="route-card">
            ...
            <Card.Body>
              <ListGroup>
                <ListGroupItem>
                  <p>
                    <span id="bold-text">Distance:</span>{" "}
                    {route[0]["distance"].toFixed(2)} mi
                  </p>
                </ListGroupItem>
                <ListGroupItem>
                  <p>
                    <span id="bold-text">Elevation Gain:</span>{" "}
                    {route[0]["total_elevation_gain"].toFixed(2)} ft
                  </p>
                </ListGroupItem>
                <ListGroupItem>
                  <p>
                    <span id="bold-text">Occurances:</span> {route.length}
                  </p>
                </ListGroupItem>
                <ListGroupItem>
                  <p>
                    <span id="bold-text">Activity Type:</span>{" "}
                    {route[0]["type"]}
                  </p>
                </ListGroupItem>
              </ListGroup>
  ...
  render() {
    ...
    return (
      <div>
        ...
        <h2 className="routes-description">
          Any similar routes that you have completed three or more times will
          appear below
        </h2>
